...
class ReduxAction(NamedTuple):
    ...
    def to_typescript_function(self) -> str:
        if isinstance(self.schema, Schema):
            contract = schema_to_contract(self.schema)
            schema_name = contract.name
        elif is_dataclass(self.schema):
            schema_name = self.schema.__name__
        else:
            raise Exception(f"what to do with {self.schema}")

        return f"""export function {self.name}(requestBody: {schema_name}): any {{
